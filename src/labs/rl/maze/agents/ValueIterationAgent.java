    ...
    extends StochasticAgent
{
    ...
    public void valueIteration(StateView state) {
        Map<Coordinate, Double> U = getZeroMap(state); // Initialize utility map
        Map<Coordinate, Double> Uprime;
        double delta;
    
        do {
            Uprime = new HashMap<>(U); // Copy U to U'
            delta = 0;
            ...
            for (Coordinate s : U.keySet()) {
                if (!isTerminalState(s)) {
                    ...
                    for (Direction a : TransitionModel.CARDINAL_DIRECTIONS) {
                        ...
                        Set<Pair<Coordinate, Double>> transitions = TransitionModel.getTransitionProbs(state, s, a);
                        for (Pair<Coordinate, Double> transition : transitions) {
                            Coordinate next = transition.getFirst();
                            double prob = transition.getSecond();
                            expectedUtility += prob * Uprime.getOrDefault(next, RewardFunction.getReward(next));
                        }
                        maxUtility = Math.max(maxUtility, expectedUtility);
                    }
                    U.put(s, RewardFunction.getReward(s) + GAMMA * maxUtility);
                    delta = Math.max(delta, Math.abs(U.get(s) - Uprime.get(s)));
                } else {
        ...
        this.setUtilities(U); // Set the final utilities
    }
